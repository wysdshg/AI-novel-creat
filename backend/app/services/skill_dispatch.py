...
- **互斥类**（文风/结构/视角/口吻/禁忌）同类只留 priority 最高的一个——
  这类 SKILL 相互影响，同时生效必然冲突或相互挤兑（多条高压「禁忌」禁令
  叠加会把模型逼出成语接龙、无标点长段之类的病态输出）；
- **叠加类**（通用）全部保留——通用是未分类兜底，强行互斥会误伤；
...
# 只有「通用」可以无限叠加；其余分类（含「禁忌」）一律互斥。
# 「禁忌」本质是硬性禁止清单，多条高压禁令同时砸向模型会相互挤兑、
# 导致输出失控（成语接龙 / 无标点长段等），因此也按互斥处理，只保留最高优先级那条。
NON_EXCLUSIVE_CATEGORIES = {"通用"}
